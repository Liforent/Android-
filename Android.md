...
- px  
  ...
  pixel像素,画面显示的基本单位,没有实际的固定的长度,只是一个抽象的取样. 目前主流手机高1920px,宽1080px
...
- dpi
  ...
  dots per inch,像素密度,单位尺寸的像素数量. dpi = 横向分辨率 / 横向英寸数
...
- Theme
  ...
  - colorPrimary 标题栏
  - colorAccent 默认光标,选中的RadioButton
...
Android的通知,在各个系统版本更新较为频繁,并且各个厂商还有自己的定制,开发时尤其需要注意不同API等级和不同厂商的适配.
...
1. 悬浮通知(提醒式通知)
   ...
   2.  1. 手机已解锁
       2. Activity处于全屏模式(FullScreenIntent)
       3. 26以下prioriy为high和以上,并且使用铃声或者震动,
       4. 26及以上,importance为high和以上
...
3. 下拉抽屉通知
   ...
   1. 通知栏下拉后可以展示抽屉通知,会一直存在直到用户关闭.
   2. 前台服务的通知不能像其他通知一样关闭,必须停止服务.
   3. API27开始优化了通知的提醒频率,一秒内发送了多条通知提示音只会触发一次.

4. 桌面角标

   1. 官方API不支持展示角标数字,要根据不同厂商适配
   2. API26开始支持长按展示通知
...
- 可快捷操作(比如暂停,播放,快捷回复等按钮)
- 带进度条

### 内置Style

API24开始增加了一些内置的模版,可以使用内置模块快速展示UI.

以IM通知样式模板为例

```Kotlin
// Android9.0 (28) 开始,必须使用Person类,才能展示头像
...
val notification = NotificationCompat.Builder(this, CHANNEL_ID)    
       ...
       .setStyle(NotificationCompat.MessagingStyle(user) 
            .addMessage(messages[1].getText(),
                   messages[1].getTime(),messages[1].getPerson())  
            .addMessage(messages[2].getText(),
                    messages[2].getTime(), messages[2].getPerson()))   
        .build()
```
...
API26及以上,必须为所有通知分配渠道,**否则通知将不会展示**.(26以下相当于是只有一个渠道了不用设置.)

可以通过NotificationChannel维度来控制通知的展示,优先级,声音和震动效果等.

### 常见属性

- Importance/Priority
  -  重要程度,决定视觉和听觉上的效果,API26以下由priority决定,26及以上由channel的importance决定.
- Visibility
  -  锁屏通知的可见详情级别.
- Category
  -  系统范围类别,默认的Category有ALARM,CALL,MESSAGE,REMINDER等,用户启动勿扰模式之后,会根据Category来设置通知的干扰等级.
...
Android原生默认不支持设置角标(支持设置红点).

- 部分厂商对Launcher进行了定制之后才有的设置角标的能力.
- 部分厂商需要提交工单或者加入白名单,才支持设置角标.
- 部分厂商的角标与通知绑定(小米等),收到通知后角标会+1

目前项目中对华为/荣耀,VIVO,OPPO/一加,三星,HTC进行了适配,其他未适配.

### 各个厂商设备适配问题

#### 华为/荣耀

- nova12, HarmonyOS 4.2.0 ,setSmallIcon不生效, 不设置的话无法展示通知,但是设置了不生效,是默认使用应用的icon, 设置了MessageStyle之后,可以正常展示消息发送者的头像.
...
- API24
  ...
  - 支持内嵌回复
- API26
  - 增加**NotificationChannel**,允许按照**NotificationChannel**关闭通知
- API33
  - 添加运行时权限
- API34
  - 全屏Intent通知
...
1. 部分手机比如华为,退到后台数秒之后,会收起原本收缩了的软键盘,此时如有必要,需要重新弹窗.应当在onFoucusChanged回调里弹软键盘,不能在onResume里弹软键盘,onFoucusChanged比onResume感知更加敏锐且不会被弹窗等影响.
...
阿里的启动优化框架https://github.com/alibaba/alpha

基于PERT图构建的异步启动框架,简单高效功能完善.
...
## Arouter
 ...
 2. 使用进阶
    ...
    - 传参数
      1. ` @JvmField
         @Autowired
         var url = "" `
      2. `url = intent.getStringExtra("url").toString()`
 ...
 3. 问题记录
    ...
    1. 无法跳转也不报错：
       ...
       最后换真机能跳转，可能是因为虚拟机是api33的。

       2. 提示there is no matched route...
...
/Glaid/,意为滑行,象征图片加载的流畅体验.

特点

- 占用内存小,默认使用RGB565格式,是PicassoARGB_8888的一半
- 缓存优化
  1. 支持内存分级缓存,正在使用图片,弱引用缓存;已使用过的图片LruCache缓存
...
​	生命周期管理(与Activity/Fragment等绑定,防止内存泄露)
...
| 缓存类型                     | 存储位置                     | 范围                      |
| :--------------------------- | :--------------------------- | :------------------------ |
| **活动缓存**ActivityResourse | 内存（弱引用）,HashMap       | 页面,避免一张图片重复解码 |
| **内存缓存**                 | 内存（LRU）,LruCache         | App                       |
| **磁盘缓存**                 | 本地文件（LRU）,DiskLruCache | 持久化                    |

- 磁盘缓存策略 DiskCacheStrategy

  Glide默认是开启内存缓存和磁盘缓存的.DiskCacheStrategy可以设置缓存原始图片或者转换过的图
...
2. Glide.with()方法
   ...
   with()方法有多个重载方法,传入的参数可以分为四类Context,Activity,Fragment,View.
   ...
   // 官方文档注释可知,with方法是开启了一个load,并且与传进来的参数绑定了生命周期.
   public static RequestManager with(FragmentActivity activity) {
     return getRetriever(activity).get(activity);
   }
   // 多个重载方法的区别如下
   ---- 如果加载的结果需要在子Fragment中使用,则使用Glide.with(该子Fragment).
   ---- 如果加载的结果需要在父Fragment中使用,则使用Glide.with(该父Fragment).
   ---- 如果需要在Activity的View中使用,则使用Glide.with(该Activity).
   ---- 如果需要再Fragment或者Activity的生命周期之外,比如在Service中或者作为Notification的缩略图使用,则使用Glide.with(Context)
   ---- 至于Glide.with(view),除非在自定义view中,否则尽量不要使用,因为在复杂的View层级中,会非常的低效.
   ```

   1. getRetriever()方法获取了Glide单例,并最终返回RequestManagerRetriever对象.
   2. RequestManagerRetriever.get(activity),会得到RequestManager对象,这个对象与生命周期相关.

3. 生命周期绑定实现原理

   在Activity里添加无UI的Fragment,通过Fragment接收Activity传递的生命周期.Fragment和RequestManager基于LifeCycle接口建立联系,并传递生命周期事件,实现生命周期感知.

4. 绑定流程

   ```Java
   17:44:20.273 27329-27329 类class(ImDeliveryExtKt.kt:57)                 pxb7.com                  E  =======zss Action(actionCode=, type=api, displayStyle=0, needConfirm=true, confirmTitle=312, confirmText=<p><img src="https://pxb7-oss-test.pxb7.com/pxb7-test/im/image/20250701/1751354611255.png" alt="" data-href="" width="" height="" style=""/><img src="https://pxb7-oss-test.pxb7.com/pxb7-test/im/image/20250630/1751286911515.jpg" alt="" data-href="" width="" height="" style=""/></p><p>2312</p>)
...
1. Glide设置圆角不准确的问题
2. Glide因为生命周期问题导致的崩溃

### 面试题

1. 如何避免Glide加载OOM?

   - 使用override(width,height)限制图片尺寸. (如何计算加载一张图片所需要的内存?)
   - 使用DiskCachesStrategy.RESOURCE缓存压缩后的图片
   - 禁止动画
...
3. Glide的缓存key如何生成?
   ...
   由图片URL,宽高,签名等参数通过哈希算法生成唯一key

4. 为何Glide默认启用RGB_565?相比ARGB_8888有什么优点?

   内存

5. 如果让你自己设计一款图片加载框架,你会从哪些角度去考虑?
...
1. 基于mmap内存映射+protobuf编码,高性能,避免IO阻塞
2. 多进程安全(文件锁和CRC校验)
3. 数据加密.
4. 无大小限制(SP有XML解析限制)
5. 写入时自动同步到文件,即使进程崩溃也不会丢失数据.
6. 增量更新.(SP全量覆盖)
7. 支持备份和恢复.
...
val response2 = client.newCall(request).enqueue(object: Callback {
  ...
})
```
...
构建者模式构建,可以在这里设置超时时间,缓存目录,拦截器,sslSocket,主机名验证等.

~~~kotlin
//通用Builder,设置了超时,缓存,以及部分拦截器
fun getBuilder():OkHttpClient.Builder {
  return OkHttpClient.Builder()
  	.readTimeout(60, TimeUnit.SECONDS)
            ...
            .addInterceptor(ChuckerInterceptor(AppGlobal.sApplication))
...
// 一般都配置上忽略主机名验证
 val defaultClient: OkHttpClient by lazy {
        ...
        return@lazy builder.sslSocketFactory(sslContext.socketFactory, trustAllCerts[0] as X509TrustManager)
...
默认最大并发请求量 maxRequests = 64

默认单个host支持的最大并发量 maxRequestsPerHost = 5
...
1. 应用拦截器 ApplicationInterceptor

   interceptors.add(client.interceptors)

   原始请求,可以添加一些自定义header、通用参数、参数加密、网关接入等等。

   它在重定向拦截器之前,只会被调用一次,后续的拦截器可能会因为重定向而执行多次.

   可以多次调用chan.proceed(本地异常重试)或者不调用(中断).
...
4. CacheInterceptor
   ...
   基于Http网络协议的数据缓存策略(参考[Http缓存原理]())

   - OkHttpClient也可以设置缓存,但是是全局的.CacheControl类的API可以实现对某个特定的request使用.
   - OkHttp默认只支持Get请求的缓存

   OKhttp缓存实现

   1. 第一次拿到响应后根据头信息决定是否缓存
   2. 下次请求时判断是否存在本地缓存,是否需要使用对比缓存,封装请求头信息等.
   3. 如果缓存失效,或者需要对比缓存,则发出网络请求,否则使用本地缓存.
   4. 使用Cache类配置缓存目录和大小
   5. 通过Cache-Control控制缓存行为(如max-age = 60)

5. ConnectionInterceptor

   连接拦截器，内部会维护一个连接池CoonectionPool，负责连接复用、创建连接（三次握手等等）、释放连接以及创建连接上的socket流。

   在连接拦截器之后的拦截器,最多只能调用一次proceed,而在这之前可以调用多次.

6. NetworkInterceptors

   interceptors.add(client.networkInterceptors)

   用户自定义拦截器，通常用于监控网络层的数据传输。这一层可以获取到最终发送请求的request,也可以获取到真正的网络请求response响应.

   - 对于从缓存中获取的response则不会去触发NetworkInterceptor,因为响应会从CacheInterceptor返回.
   - 观察数据在网络中的传输
   - 可以获得装在请求的链接

7. CallServerInterceptor

   请求拦截器，在前置准备工作完成后，真正发起了网络请求。经过责任链的一层层递推下去,最终会执行到CallServerInterceptor的intercept方法,这个方法会将网络响应结果封装成Response对象并且沿着责任链一级一级回溯.最终回到getResponseWithInterceptorChain方法的返回.
...
4. 得到response
   ...
   RealCall#execute()方法会返回Response
...
1. get请求流程
   ...
   1. 构建OkHttpClient对象.
      ...
      public static final class Builder {
        ...
        SSLSocketFactory sslSocketFactory;//安全套接字socket工厂,用于HTTPS.
        HostnameVerifier hostnameVerifier;//验证确认响应证书 使用HTTPS请求连接的主机名
   ...
   2. 建造者模式构建Request对象.
      ...
      1. 构建RealCall对象(client.newCall(request))并发送网络请求.

      ```Java
      //ReallCall是请求的真正执行者
      final class RealCall implement Call {
        final OkHttpClient client;
        final Request originalRequst;
      ...
      //同步请求的具体逻辑execute()
        ...
        //OkHttp责任链模式的核心.
        Response getResponseWithInterceptorChain() throws IOExceptions {
          ...
          //构建interceptors数组并添加好一堆拦截器. 
          //自定义拦截器,最先添加的最后执行.  
          interceptors.addAll(client.interceptors());
          //重试和重定向拦截器
          interceptors.add(new RetryAndFollowUpInterceptor(client));
          //桥接拦截器,主要对 Request 中的 Head 设置默认值，比如 Content-Type、Keep-Alive、Cookie 等。
          interceptors.add(new BridgeInterceptor(client.cookieJar()));
          //缓存拦截器
          interceptors.add(new CacheInterceptor(client.internalCache()));
          //连接拦截器,负责建立与服务器地址之间的连接，也就是 TCP 链接。在ConnectInterceptor以及之后的拦截器最做只能调用一次proceed.
          interceptors.add(new ConnectInterceptor(client));
          if (!forWebSocket) {
            //注意这里是netWorkInterceptors
            interceptors.add(client.networkInterceptors());
          }
          //请求拦截器,负责向服务器发送请求，并从服务器拿到远端数据结果。
        ...
        // RealInterceptorChain.java  proceed就是继续的意思,依次执行每个拦截器,获取请求体.里边确保了最后一个拦截器必须是CallServerInterceptor,并且每个拦截器里都执行了proceed().
        public Response proceed(...) {
          ...
          Interceptor interceptor = interceptors.get(index);
           ...   
           //Interceptor.intercept()方法里最终获取到了response,而Interceptor是抽象类,intercept()方法需要自己子类自己实现.  
...
okhttp使用ConnectionPool实现连接TCP连接复用,避免频繁的创建和握手挥手的开销.

#### 复用条件

​	相同Host的请求,http1.1默认开启Connection:keep-alive(长连接),Http2默认支持多路复用,一个连接可并发多个请求.

- 什么情况下连接不会被复用?
  1. 不同Host
  2. Connection.close显式关闭
  3. 连接超时或者异常关闭
  4. TLS配置不同(比如证书不同)

#### 默认配置

​	最大空闲连接数为5个,空闲连接存活时间为5分钟,

#### 连接复用流程

1. 发起请求
2. 检查ConnectionPool里是否有相同Host的空闲连接
3. 若有直接复用,否则新建连接.
...
2. 调整keep-alive时间
3. 使用http/2
4. 避免频繁创建OKhttpClient比如使用单例.

### OKHttp如何支持HTTPS?

默认支持https,但可以自定义SSLSocketFactory和HostnameVerifier来跳过Host验证.
...
1. 简述OkHttp整体请求流程
   ...
   2. OkHttpClient通过newCall发起网络请求
   3. 通过Dispatcher维护请求队列与线程池,完成请求配对
   4. 通过五个默认拦截器完成请求重试,缓存处理,建立连接等
   5. 得到网络请求结果
   ```
...
2. OkHttp分发器是怎么工作的?
   ...
   ```kotlin 
   1. 同步请求Dispathcer#execute()
   	不需要线程池,分发器仅做一下记录,按照加入队列的顺序同步请求即可.
   2. 异步请求Dispatcher#enqueue()
   	设定正在执行的任务maxRequest<64,同时同一Host请求runningCallsForHost<5,满足直接添加到正在执行队列,同时提交给线程池.否则先加入等待队列.
   Dispatcher#finish()会取出等待队里中的任务继续执行.
   ```

3. 拦截器是怎么工作的?有哪些拦截器?

   ```kotlin 
   责任链的从RealCall#execute()方法里开启 getResponseWithInterceptorChain()
   应用,重试重定向,桥接,缓存,连接,网络,请求服务
   ```

4. OkHttp如何复用TCP连接

   ```kotlin
   ConnectionInterceptor负责建立TCP连接.
   ```
...
- 功能强大
  支持同步,异步,支持多种数据的解析和格式化,支持RxJava

- 简洁易用

  通过注解配置请求网络参数,采用大量设计模式简化使用

- 可拓展性好

  功能模块高度封装,解耦彻底,如自定义Converters等.
...
1. 使用AppCompact等官方兼容库

2. 使用反射(慎用)

3. 判断Build.VERSION.SDK_INT,自己实现低版本逻辑

4. 使用编译时注解,说明代码生效范围或者跳过lint检查.

   - @TargetApi(...)

     代码只在目标API以上生效,防止低版本编译错误.

   - @RequiresApi(...)

     与上边类似,但更严格,要求目标API在编译时就满足

   - @SuppressLint("NewApi")

     人哦啊过Lint检查,但仍需要在运行时处理版本兼容问题.
