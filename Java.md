...
1. 对象优先被分配到Eden区,如果是大对象,直接进入老年代.
2. 当Eden区空间不足时,JVM执行Minor GC
3. Eden区和FromSurvivor(0)区存活的对象,会被赋值到To Survivor(1),对象年龄+1
4. 如果对象年龄达到阈值(一般15),或者To Survivor空间不够,会直接进入老年代.
5. 清空Eden和From Survivor.
6. 交换To Survivor和From Survivor.

上述是Minor GC,通常非常快(几十毫秒内)

Major GC(Full GC,老年代)

FullGC通常是指对新生代,老年代,元空间进行全局回收,触发条件:

1. 调用System.gc()建议JVM执行FullGC.
2. 老年代控件不足
3. 堆内存分配失败等.
...
2. **框架开发的基础**：
   ...
   热修复,插件化,Retrofit,Serializable等.
...
运行期间生成代理类,有反射开销,只需要编写一个通用处理器,接口变化自动适配.

Retrofit的网络接口类就是通过动态代理实现的.
...
val service = Retrofit.Builder()
    ...
    .build()
    .create(ApiService::class.java) // 内部通过动态代理生成实现
~~~
...
1. JDK动态代理
   ...
   JDK 动态代理要求目标对象**必须实现接口**，通过 `java.lang.reflect.Proxy` 和 `InvocationHandler` 生成代理对象。

2. CGLIB动态代理

3. 编译时代理(类似动态代理的思想)

   Dagger/Hilt,Room,ViewBing等.

## 静态代理

编译期间创建了代理类,需要为每个目标编写代理类,接口变化需要同步修改代理类,无反射开销.
...
- 底层BinderProxy是动态代理
  ...
  **`BinderProxy`**是在程序运行时,由Binder驱动根据需要动态生成的

- **`Stub.Proxy`**是封装好的静态代理对类

  封装了所有与BinderProxy通讯的细节.
